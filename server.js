...
const PHONE_HTML = 
  ...
  "async function startScreenShare() { try { mediaStream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: false }); startStreamingWithMedia(); } catch (e) { document.getElementById('action-toast').innerText = 'Screen capture canceled or unsupported.'; } }" +
...
server.listen(PORT, () => {
  console.log(`🚀 HYBRID WEB & ADB SERVER RUNNING ON PORT ${PORT}`);
});

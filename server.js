...
const EMBEDDED_CSS = 
  ...
  ".modal-content { width: 100%; max-width: 460px; padding: 30px; }" +
...
const INDEX_HTML = 
  ...
  "<button class='btn-primary' onclick='openPairingModal()'>+ Connect Link / PIN</button><div class='connection-pill'><span id='server-status-dot' class='status-dot connecting'></span><span id='server-status-text'>Connecting...</span></div></div></header>" +
  ...
  "<div style='font-weight: 700; font-size: 0.85rem; margin-bottom: 4px; color: #10b981;' id='mode-info-title'>⚡ Zero-Code Instant Active</div><div style='font-size: 0.78rem; color: #94a3b8; line-height: 1.4;' id='mode-info-desc'>Phones connect instantly without typing any PIN code! Tap header button to switch mode.</div></div></div></div>" +
  ...
  "<div id='stream-placeholder' class='stream-placeholder'><div style='font-size: 58px; color: #6366f1;'>📲</div><h3 style='font-size: 1.35rem; color: #fff;'>No Mobile Stream Active</h3><p style='max-width: 420px; font-size: 0.9rem;'>Open your website URL on your mobile phone to connect live.</p><button class='btn-primary' style='margin-top: 10px;' onclick='openPairingModal()'>View Mobile Connection Link</button></div>" +
  ...
  "<div id='pairing-modal' class='modal-overlay'><div class='glass-panel modal-content'><div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;'><h3>Mobile Device Connection Link</h3><button style='background: none; border: none; color: #fff; font-size: 1.5rem; cursor: pointer;' onclick='closePairingModal()'>×</button></div>" +
  "<div style='font-size: 0.88rem; color: #94a3b8; margin-bottom: 16px;'><b>🌐 Open this Link on your Mobile Phone:</b><code id='phone-url-display' style='background: rgba(0,0,0,0.4); padding: 8px 12px; border-radius: 6px; color: #10b981; font-weight: 600; display: block; margin-top: 6px; word-break: break-all;'>http://loading...</code><button class='btn-primary' style='margin-top: 8px; width: 100%; font-size: 0.85rem;' onclick='copyPhoneUrl()'>📋 Copy Link to Send</button></div>" +
  "<div style='text-align: center; margin: 16px 0; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 16px;'><div style='font-size: 0.85rem; color: #94a3b8; margin-bottom: 10px;'>- Or Enter 6-Digit PIN Code (PIN Security Mode) -</div><div class='pin-inputs'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 0)'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 1)'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 2)'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 3)'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 4)'><input type='text' class='pin-digit' maxlength='1' onkeyup='handlePinInput(this, 5)'></div><button class='btn-primary' style='width: 100%; margin-top: 10px;' onclick='submitPairingCode()'>Authorize via PIN Code</button></div><div id='pending-pairing-alerts'></div></div></div>" +
  "<script>" +
...
const PHONE_HTML = 
  ...
  "<div class='connection-pill' style='justify-content: center; margin-bottom: 20px;'><span id='phone-status-dot' class='status-dot connecting'></span><span id='phone-status-text'>Connecting...</span></div>" +
  ...
  "<div id='authorized-card'><div style='padding: 14px; background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.3); border-radius: 12px; margin-bottom: 20px; color: #10b981; font-weight: 600;'>🟢 Authorized & Connected!</div>" +
  "<button id='btn-start' class='btn-primary' style='width: 100%; padding: 14px; font-size: 1rem;' onclick='startScreenCapture()'>🎬 Start Screen Stream</button>" +
  "<button id='btn-stop' style='display: none; width: 100%; padding: 14px; background: #ef4444; color: #fff; border: none; border-radius: 10px; margin-top: 10px; font-weight: 600;' onclick='stopScreenCapture()'>⏹️ Stop Streaming</button>" +
  ...
  "<div id='action-toast' style='margin-top: 20px; font-size: 0.85rem; color: #6366f1;'></div></div>" +
  "<script>" +
  ...
  "async function startScreenCapture() { try { if (!navigator.mediaDevices || !navigator.mediaDevices.getDisplayMedia) { alert('Screen Stream API not supported on this browser version. Please update Chrome.'); return; } mediaStream = await navigator.mediaDevices.getDisplayMedia({ video: { cursor: 'always' }, audio: false }); document.getElementById('btn-start').style.display = 'none'; document.getElementById('btn-stop').style.display = 'block'; const canvas = document.getElementById('preview'), ctx = canvas.getContext('2d'); canvas.style.display = 'block'; const v = document.createElement('video'); v.srcObject = mediaStream; v.play(); v.onended = () => stopScreenCapture(); captureInterval = setInterval(() => { if (v.readyState !== v.HAVE_ENOUGH_DATA) return; canvas.width = v.videoWidth * 0.75; canvas.height = v.videoHeight * 0.75; ctx.drawImage(v, 0, 0, canvas.width, canvas.height); canvas.toBlob((b) => { if (b && ws && ws.readyState === 1) b.arrayBuffer().then(buf => ws.send(buf)); }, 'image/jpeg', 0.7); }, 33); } catch (e) {} }" +
  "function stopScreenCapture() { if (captureInterval) clearInterval(captureInterval); if (mediaStream) mediaStream.getTracks().forEach(t => t.stop()); document.getElementById('btn-start').style.display = 'block'; document.getElementById('btn-stop').style.display = 'none'; }" +
...
server.listen(PORT, () => {
  console.log(`🚀 DEVICESTREAM PRO V2.0 ULTRA SELF-CONTAINED RUNNING ON PORT ${PORT}`);
});

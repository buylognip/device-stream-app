...
const PHONE_HTML = 
  ...
  "</div>" +
  "<div style='font-size: 0.76rem; color: #94a3b8; background: rgba(255,255,255,0.04); padding: 10px; border-radius: 8px; line-height: 1.3;'>💡 <b>Samsung Galaxy A16 Tip</b>: For full screen mirror, tap Chrome's top-right 3 dots ⁝ -> check <b>Desktop site</b>.</div>" +
  ...
  "async function startScreenShare() { try { mediaStream = await navigator.mediaDevices.getDisplayMedia({ video: { cursor: 'always' }, audio: false }); startStreamingWithMedia(); } catch (e) { alert('To mirror your screen on Galaxy A16: Click Chrome top-right 3 dots ⁝ -> check Desktop site, then tap Mirror Phone Screen!'); } }" +
...
server.listen(PORT, () => {
  console.log(`🚀 DUAL MODE SCREEN & CAMERA MIRROR RUNNING ON PORT ${PORT}`);
});
